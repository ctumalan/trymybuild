const projects = [
  { slug: "afterschooltogether", name: "AfterSchool Together", category: "Family life", icon: "A", color: "coral", summary: "Find out whether one adult can actually make every after-school trip.", purpose: "Turns a week of children's activities into the drop-offs and pickups it really requires, then checks them against your own travel times.", audience: "Parents coordinating several children's activities", stage: "New", price: "Free", url: "https://afterschool-together-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Check whether your week's drop-offs and pickups actually work", note: "Made in-house by the TryMyBuild team. This is a new listing and community evidence has not been collected yet. It works entirely in your browser: no account, no cloud sync, and no maps or live traffic — travel times are the ones you enter." },
  { slug: "stackscout", name: "StackScout", category: "Technology", icon: "S", color: "teal", summary: "Compare technology choices without drowning in technical language.", purpose: "Helps you narrow down the right tools for a project by comparing what matters most.", audience: "People choosing technology for a new idea", stage: "New", price: "Free", url: "https://stack-scout-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Choose a practical technology direction", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "gamegrid", name: "GameGrid", category: "Sports & teams", icon: "G", color: "blue", summary: "Turn scattered game details into one clear team schedule.", purpose: "Keeps practices, games, locations, and team plans together so fewer details get lost.", audience: "Coaches, players, and team organizers", stage: "New", price: "Free", url: "https://game-grid-app-xi.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Build and review a sports schedule", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "lessonlab", name: "LessonLab", category: "Teaching & learning", icon: "L", color: "gold", summary: "Build a balanced lesson around the time you actually have.", purpose: "Helps teachers shape a lesson with a clear beginning, activity, and close.", audience: "Teachers, tutors, and workshop leaders", stage: "New", price: "Free", url: "https://lesson-lab-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Create a timed lesson plan", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "cartcompare", name: "CartCompare", category: "Shopping", icon: "C", color: "coral", summary: "Compare what purchases really cost before you decide.", purpose: "Places prices and practical differences side by side for a calmer buying decision.", audience: "Anyone comparing products or shopping options", stage: "New", price: "Free", url: "https://cart-compare-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Compare the true cost of several choices", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "pocketbalance", name: "PocketBalance", category: "Money", icon: "P", color: "green", summary: "See your month clearly without building a complicated budget.", purpose: "Creates a simple snapshot of money coming in, going out, and remaining.", audience: "People who want a gentler view of monthly finances", stage: "New", price: "Free", url: "https://pocket-balance-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Create a monthly financial snapshot", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "dayframe", name: "DayFrame", category: "Personal planning", icon: "D", color: "violet", summary: "Plan a day that respects your actual time and energy.", purpose: "Turns a long task list into a realistic daily plan with room to breathe.", audience: "Busy people who want a more realistic day", stage: "New", price: "Free", url: "https://day-frame-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Shape a realistic plan for today", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "mealmap", name: "MealMap", category: "Food & home", icon: "M", color: "coral", summary: "Make a practical weeknight meal plan from what works for you.", purpose: "Reduces the daily question of what to cook by mapping meals across the week.", audience: "Households planning everyday meals", stage: "New", price: "Free", url: "https://meal-map-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Create a weeknight meal plan", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "homerhythm", name: "HomeRhythm", category: "Food & home", icon: "H", color: "gold", summary: "Keep small home-maintenance jobs from becoming big surprises.", purpose: "Organizes recurring household care into a schedule you can actually follow.", audience: "Renters and homeowners managing a household", stage: "New", price: "Free", url: "https://home-rhythm-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Build a home-maintenance rhythm", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
  { slug: "packlight", name: "PackLight", category: "Travel", icon: "P", color: "blue", summary: "Pack for a trip without carrying your whole closet.", purpose: "Builds a focused carry-on list around the trip, weather, and activities.", audience: "Travelers who want to pack lighter", stage: "New", price: "Free", url: "https://pack-light-app.vercel.app", linkNote: "Opens the creator's website", accessNote: "No sign-in needed to try it", outcome: "Create a practical carry-on list", note: "This is a new TryMyBuild listing. Community evidence has not been collected yet." },
...
function selectHomeView(view) {
  ...
  state.entryMode = homeView === 'test' ? 'list' : 'search';
...
function homepageAppUrl(value) {
  ...
  const candidate = /^https?:\/\//i.test(raw) ? raw : /^(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z]{2,}(?::\d+)?(?:[/?#].*)?$/i.test(raw) ? 'https://' + raw : '';
  if (!candidate) return '';
...
function discover(communityFocused = false) {
  ...
  if (activeView === 'test') return `<section class="page-shell discover-page future-discover listing-in-place">${discoveryHero(true)}<div id="home-panel" class="listing-entry-panel" aria-label="Share your app">${listingJourney()}</div></section>`;
  if (window.CW_SERVER && catalogState !== 'ready') return `<section class="page-shell discover-page future-discover">${discoveryHero()}<div id="home-panel" tabindex="-1">${catalogStatusPanel()}</div></section>`;
  ...
  const query = state.entryMode === 'search' ? state.query : '';
...
function maybeShowReturnFeedback() {
  ...
  dialog.showModal();
  window.CWGuidedFeedback?.hydrate(dialog);
...
function render(preserveScroll = false) {
  ...
  const listingPanel = app.querySelector('.listing-entry-panel');
  if (listingPanel && (state.route === 'share' || (state.route === 'discover' && homeView === 'test'))) {
    // Advance only the form: keep the familiar navigation in place through every step.
    const template = document.createElement('template');
    template.innerHTML = content;
    listingPanel.innerHTML = template.content.querySelector('.listing-entry-panel').innerHTML;
  } else app.innerHTML = content;
  syncQuoteVisibility();
...
document.addEventListener("click", async event => {
  ...
  if (save) {
    ...
    if (window.CW_SERVER) {
      if (!state.session?.authenticated) { window.CWJoin.offer({action:'save',slug:save.dataset.save,project:save.closest('.detail-dialog')?save.dataset.save:undefined,browse:{category:state.category,query:state.query,price:state.price,sort:state.sort,creatorType:state.creatorType,verifiedOnly:state.verifiedOnly}}); return; }
      save.disabled = true;
      try {
  ...
  if (event.target.closest("[data-feedback-submit]")) {
    ...
    if (window.CW_SERVER) {
      if (!state.session?.authenticated) { saveProjectCommentDraft(state.selected.slug,response); window.CWJoin.offer({action:'comment',slug:state.selected.slug,project:state.selected.slug}); return; }
...
function loadCatalog() {
  ...
  fetch('/api/catalog').then(r => r.ok ? r.json() : Promise.reject(new Error('unavailable'))).then(data => {
    ...
    openLinkedProject();window.dispatchEvent(new Event('cw-catalog-ready'));
  }).catch(() => {
    catalogState = 'error';
    if (['discover', 'community'].includes(state.route) && !document.querySelector('.detail-dialog')) render();
  });
...
window.CWOpenProject = slug => { const project = projects.find(p => p.slug === slug); if (project) openProductDetail(project); return !!project; };
...
window.addEventListener('cw-saved-changed',async()=>{try{const data=await fetch('/api/saved').then(r=>r.json());if(!Array.isArray(data.saved))return;state.saved=new Set(data.saved);document.querySelectorAll('[data-save]').forEach(b=>{const saved=state.saved.has(b.dataset.save);b.textContent=saved?'♥ Saved':'♡ Save';b.classList.toggle('is-saved',saved);});}catch{}});

window.CWBrowseContext = () => ({category:state.category,query:state.query,price:state.price,sort:state.sort,creatorType:state.creatorType,verifiedOnly:state.verifiedOnly});
