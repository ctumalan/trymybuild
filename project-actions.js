...
if(!document.getElementById('app')){
 ...
 async function returned(){
  ...
  const kind=elapsed<15000?'quick':elapsed>120000?'detailed':'comment';if(window.CWReturnPrompts?.suppressed(kind)||prompted.has(slug+kind)||document.querySelector('.invitation-dialog[open],.return-feedback-dialog[open]'))return;prompted.add(slug+kind);
  if(kind==='detailed'){window.CWGuidedFeedback.open(slug);return;}
  if(kind==='quick'){const dialog=document.createElement('dialog');dialog.className='return-feedback-dialog';const choices=[['too_much_to_read','Too much to read'],['hard_to_understand','Hard to understand'],['unexpected','Not what I expected'],['curious','Just curious']];dialog.innerHTML='<h2>A quick thought?</h2><p>Anything that made you stop exploring?</p><form><fieldset><legend>Select any that fit</legend>'+choices.map(([v,l])=>'<label><input type="checkbox" name="reason" value="'+v+'">'+l+'</label>').join('')+'</fieldset><button class="primary-button" type="submit">Send response</button><p role="status"></p></form>'+window.CWReturnPrompts.option()+'<button type="button" data-dismiss>Not now</button>';document.body.append(dialog);dialog.showModal();const close=()=>{dialog.close();dialog.remove();};dialog.querySelector('[data-dismiss]').onclick=close;dialog.addEventListener('cancel',e=>{e.preventDefault();close();});dialog.querySelector('form').onsubmit=async e=>{e.preventDefault();const reasons=[...dialog.querySelectorAll('input[name=reason]:checked')].map(i=>i.value),status=dialog.querySelector('[role=status]');if(!reasons.length){status.textContent='Choose an option first.';return;}const button=dialog.querySelector('[type=submit]');button.disabled=true;try{const r=await fetch('/api/quick-feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({slug,reasons,requestId:dialog.dataset.requestId||=crypto.randomUUID()})}),data=await r.json();if(!r.ok)throw Error(data.error);status.textContent=data.message;dialog.querySelector('[data-dismiss]').textContent='Done';}catch(error){status.textContent=error.message;button.disabled=false;}};return;}
  const dialog=document.createElement('dialog');dialog.className='return-feedback-dialog';dialog.innerHTML='<button type="button" class="return-feedback-close" aria-label="Close feedback">×</button><h2>How was the app?</h2><p>Leave a public comment about what worked or could be better.</p><section><form class="detail-comment-form compact-comment" data-public-comment="'+slug+'"><div class="comment-input-wrap"><label for="return-public-comment">Your comment</label><textarea id="return-public-comment" name="comment" rows="3" maxlength="800" required></textarea><button class="primary-button" type="submit" hidden>Send comment</button></div><small data-project-comment-count></small><p data-comment-status role="status"></p></form><p class="cw-meta">Guests appear as Guest. Public after review. One comment per app; sign in to update yours.</p><div data-public-comments></div></section>'+window.CWReturnPrompts.option()+'<button type="button" data-dismiss>Not now</button>';document.body.append(dialog);dialog.showModal();const close=()=>{dialog.close();dialog.remove();};dialog.querySelector('[data-dismiss]').onclick=close;dialog.querySelector('.return-feedback-close').onclick=close;dialog.addEventListener('cancel',event=>{event.preventDefault();close();});window.dispatchEvent(new Event('cw-panel-ready'));dialog.querySelector('textarea').focus();
 }
