...
if(!document.getElementById('app')){
 ...
 async function returned(){
  ...
  if(kind==='detailed'){const dialog=document.createElement('dialog');dialog.className='return-feedback-dialog';dialog.innerHTML='<button class="return-feedback-close" aria-label="Close feedback">×</button><h2>What stood out?</h2><p>Tell the creator what worked and what could improve.</p><div data-feedback-body>Loading feedback form…</div>';document.body.append(dialog);dialog.showModal();const close=()=>{dialog.close();dialog.remove();};dialog.querySelector('button').onclick=close;dialog.addEventListener('cancel',e=>{e.preventDefault();close();});
   try{const r=await fetch('/tell/'+encodeURIComponent(slug));if(!r.ok)throw Error();const doc=new DOMParser().parseFromString(await r.text(),'text/html'),form=doc.querySelector('[data-guided-feedback]');if(!form)throw Error();form.querySelectorAll('script').forEach(s=>s.remove());dialog.querySelector('[data-feedback-body]').replaceChildren(form);window.CWGuidedFeedback.hydrate(dialog);}catch{dialog.querySelector('[data-feedback-body]').textContent='Feedback could not load. Open the app’s feedback page and try again.';}return;
  }
 ...
 window.addEventListener('cw-feedback-resume',event=>{if(location.pathname.startsWith('/tell/'))return;visit={slug:event.detail,awayAt:Date.now()-120001};returned();});
}
