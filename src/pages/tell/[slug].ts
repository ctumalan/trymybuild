...
export const GET:APIRoute = async context => {
 ...
 return surface(`Tell the creator · ${product.name}`,`${notice}<div class="cw-grid"><aside class="cw-project cw-panel"><a href="/projects/${e(slug)}">← Project details</a><img src="${e(product.preview)}" alt="${e(product.name)} website preview"><p class="eyebrow">${e(product.category)}</p><h2>${e(product.name)}</h2><p>${e(product.summary)}</p><p class="cw-meta">${ownerReady?'A conversation with '+e(creator):'This inbox is being connected'}</p>${url?`<a class="primary-button" href="${e(url)}" target="_blank" rel="noopener noreferrer">Try ${e(product.name)} ↗</a>`:''}<p class="cw-meta">Then come back here. Clicking is not proof of use or purchase.</p></aside><div>${trialBrief({firstTry:product.firstTry,creatorName:creator,creatorAvatar},{question:trialRequest?.question})}${form}<section><h2>What people shared</h2><p class="cw-meta">Public, self-reported feedback—not verified purchases or proof of use.</p>${publicRows.length?publicRows.map(row=>`<article class="cw-panel"><strong>${e(row.author)}</strong>${signals(row)}<p class="cw-message">${e(row.message)}</p></article>`).join(''):'<p>No public feedback yet.</p>'}${pages('/tell/'+slug+'?public=1',page,count)}</section></div></div>`);
};
