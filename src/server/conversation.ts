...
export async function conversation(db:any,userId:string,id:string,url:URL) {
 ...
 let trialRequest:any=null;
 try{const requestResult=await db.from('feedback_requests').select('id,question,status,created_at').eq('project_slug',f.project_slug).eq('user_id',project.owner_user_id).in('status',['queued','fulfilled']).lte('created_at',f.created_at).order('created_at',{ascending:false}).limit(1).maybeSingle();if(!requestResult.error)trialRequest=requestResult.data;}catch{}
 ...
 const makerHasReplied=replyResult.data.some((r:any)=>r.author_user_id===project.owner_user_id),replyPromise=userId===project.owner_user_id&&trialRequest&&!makerHasReplied?`<p class="cw-notice maker-reply-promise"><strong>This tester is waiting for you.</strong> Reply within two days. The trial is complete when you respond personally.</p>`:'';
