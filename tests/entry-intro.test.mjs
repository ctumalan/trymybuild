...
function setup({seen=false,reduced=false,width=400,value='',storageBlocked=false}={}) {
 ...
 const document={hidden:false,activeElement:null,querySelector:()=>field,
 ...
 return {input,field,document,classes,timers,storage,mount:()=>context.window.CWEntryIntro.mount(document),
...
test('intro rolls two lines then restores the untouched placeholder',()=>{
 ...
 const timer=[...s.timers.values()][0];assert.equal(timer.ms,4200);timer.fn();
...
test('async catalog rerenders continue the same timeline rather than replaying',()=>{
 ...
 assert.equal([...s.timers.values()][0].ms,2200);
});
