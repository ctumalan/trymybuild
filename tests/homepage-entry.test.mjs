...
test('search submission only moves to the catalog',()=>{const x=setup('meal planner');x.c.state.entryMode='search';x.run();assert.deepEqual(x.counts(),{saves:0,renders:0,scrolled:1});assert.equal(x.c.state.route,'discover');});
...
test('search mode never starts a listing even for a URL',()=>{const x=setup('finalprompt.ai');x.c.state.entryMode='search';x.run();assert.equal(x.c.state.route,'discover');assert.equal(x.counts().saves,0);});
test('default listing mode rejects search text without saving',()=>{const x=setup('meal planner');x.run();assert.deepEqual(x.counts(),{saves:0,renders:0,scrolled:0});});
...
test('listing questions, preview, account and settings retain the same navigation',()=>{
 ...
 const context=vm.createContext({state:{entryMode:'list'},homeView:'test',listingStep:0,listingSettings:false,
 ...
 vm.runInContext(fn('discoveryHero')+fn('discover')+fn('listingJourney'),context);
 ...
 assert.match(context.discover(true),/Find an app[\s\S]*<h1>Settings<\/h1>/);
});
...
test('browsing mid-listing and returning preserves the draft and current question',()=>{
 ...
 const end=source.indexOf("    const form = entryMode.closest",start);
